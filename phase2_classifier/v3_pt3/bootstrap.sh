#!/usr/bin/env bash
# Bootstrap a cheap CPU spot instance (Ubuntu 22.04) for Stage 2b v2 NAIP
# polygon-delta scoring. No GPU, no torch. Pure rasterio + scipy + multiprocessing.
#
# Telemetry mirrors v3_pt2: per-chunk stdout tail, JSONL stats, heartbeat,
...
echo "[bootstrap-pt3] instance=$INSTANCE_ID type=$INSTANCE_TYPE region=$REGION"

LOG_S3="s3://${BUCKET}/v3-pt3-artifacts/logs/${INSTANCE_ID}.log"
STATS_S3="s3://${BUCKET}/v3-pt3-artifacts/logs/${INSTANCE_ID}.stats.jsonl"

if [ ! -x /usr/local/bin/aws ] || ! /usr/local/bin/aws --version 2>&1 | grep -q 'aws-cli/2'; then
  echo "[bootstrap-pt3] installing AWS CLI v2"
...
hash -r
echo "[bootstrap-pt3] aws -> $(which aws) $(aws --version 2>&1)"

echo "[bootstrap-pt3] pulling bundle"
aws s3 sync "s3://${BUCKET}/v3-pt3-bundle/" ./bundle/ --only-show-errors

mkdir -p data_us/phase2/v3 data_us/phase3_naip sites_us
cp bundle/stage3_candidates_v3.parquet          data_us/phase2/v3/
cp bundle/stage2_candidate_polygons.parquet     data_us/phase2/v3/
cp bundle/naip_tile_index_baseline.parquet      data_us/phase3_naip/
cp bundle/naip_tile_index_recent.parquet        data_us/phase3_naip/
cp -r bundle/code/sites_us/*                    sites_us/
cp bundle/.env                                  sites_us/.env || true

CHUNK_DIR="data_us/phase2/v3/stage2b_naip_chunks${STAGE2B_RUN_TAG:-}"
mkdir -p "$CHUNK_DIR"
echo "[bootstrap-pt3] pulling existing chunks (resume)"
aws s3 sync "s3://${BUCKET}/v3-pt3-artifacts/stage2b_naip_chunks${STAGE2B_RUN_TAG:-}/" \
  "$CHUNK_DIR/" --only-show-errors
RESUME_N=$(find "$CHUNK_DIR" -maxdepth 1 -name 'chunk_*.parquet' 2>/dev/null | wc -l)
echo "[bootstrap-pt3] resumed chunks: $RESUME_N"

[ -f sites_us/.env ] && set -a && . sites_us/.env && set +a

echo "[bootstrap-pt3] python deps"
...
echo "[bootstrap-pt3] vCPU=$VCPU  workers=$STAGE2B_NUM_WORKERS  chunk_size=$STAGE2B_CHUNK_SIZE"

(
  for i in $(seq 1 10); do
    sleep 30
    aws s3 cp "$HOME/v3pt3.out" "$LOG_S3" --only-show-errors 2>/dev/null || true
  ...
  while true; do
    ...
    aws s3 sync "$CHUNK_DIR/" \
      "s3://${BUCKET}/v3-pt3-artifacts/stage2b_naip_chunks${STAGE2B_RUN_TAG:-}/" \
      --exclude "*" --include "chunk_*.parquet" --only-show-errors 2>/dev/null || true
  done
) &
SYNC_PID=$!
echo "[bootstrap-pt3] sync loop pid=$SYNC_PID"

(
  while true; do
    sleep 60
    aws cloudwatch put-metric-data \
      --region "$REGION" --namespace "stage2b_naip" \
...
on_exit() {
  local code=$?
  echo "[bootstrap-pt3] EXIT code=$code; final sync"
  aws s3 cp "$HOME/v3pt3.out" "$LOG_S3" --only-show-errors 2>/dev/null || true
  aws s3 sync "$CHUNK_DIR/" \
    "s3://${BUCKET}/v3-pt3-artifacts/stage2b_naip_chunks${STAGE2B_RUN_TAG:-}/" \
    --only-show-errors 2>/dev/null || true
  kill $SYNC_PID $CW_PID 2>/dev/null || true
  echo "[bootstrap-pt3] terminating $INSTANCE_ID"
  aws ec2 terminate-instances --region "$REGION" --instance-ids "$INSTANCE_ID" 2>&1 || true
}
trap on_exit EXIT

cd sites_us
python3 -u phase2_classifier/v3_pt3/polygon_delta_scan.py 2>&1 | tee -a "$HOME/v3pt3.out"
